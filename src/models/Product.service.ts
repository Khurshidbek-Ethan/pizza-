...
//     }
//     console.log("inquiry.search2", inquiry.search);
  ...
  public async getProducts(inquiry: ProductInquiry): Promise<Product[]> {
    ...
    const result = await this.productModel
      .aggregate([
        { $match: match },
        { $sort: sort },
        { $skip: (inquiry.page * 1 - 1) * inquiry.limit }, //0 //3 agar pagination 2=>1,2,3 tashavor degani
        { $limit: inquiry.limit * 1 }, //agar pagination 2 da turgan bolsa 3 =>4,5,6
      ])
      .exec();
    if (!result) throw new Errors(HttpCode.NOT_FOUND, Message.NO_DATA_FOUND);
